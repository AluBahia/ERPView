...
describe('useRH', () => {
  ...
  test('retorna colaboradores com sucesso', async () => {
    const mockData = [{ id: 1, matricula: '001', nome: 'Joao', cargo: 'Analista', departamento: 'TI', status: 'Ativo' }];
    mockSupabaseClient.from.mockImplementation(() => mockBuilder(mockData));

    const { result } = renderHook(() => useRH(), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data).toHaveLength(1);
    expect(result.current.data?.[0].nome).toBe('Joao');
  });

  test('aplica filtro de departamento', async () => {
    const builder = mockBuilder([]);
    mockSupabaseClient.from.mockImplementation(() => builder);

    renderHook(() => useRH({ departamento: 'TI' }), { wrapper: createWrapper() });
    await waitFor(() => expect(builder.then).toHaveBeenCalled());
    expect(builder.eq).toHaveBeenCalledWith('departamento', 'TI');
  });
  ...
  test('propaga erro quando Supabase falha', async () => {
    ...
    expect(result.current.error?.message).toContain('Erro ao carregar colaboradores');
  });
});
