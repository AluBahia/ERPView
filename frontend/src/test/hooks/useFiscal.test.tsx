...
describe('useFiscal', () => {
  ...
  test('retorna notas fiscais com sucesso', async () => {
    const mockData = [{ id: 1, numero: 'NF001', contraparte: 'Cliente A', valor: 5000, tipo: 'Saida' }];
    ...
    expect(result.current.data?.[0].numero).toBe('NF001');
  });
  ...
  test('propaga erro quando Supabase falha', async () => {
    ...
    expect(result.current.error?.message).toContain('Erro ao carregar notas fiscais');
  });
});
