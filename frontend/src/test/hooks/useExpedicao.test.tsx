...
describe('useExpedicao', () => {
  beforeEach(() => vi.clearAllMocks());

  test('retorna pedidos de expedicao com sucesso', async () => {
    const mockData = [{ id: 1, numero: 'EXP001', cliente: 'Cliente A', status: 'Em transito' }];
    mockSupabaseClient.from.mockImplementation(() => mockBuilder(mockData));

    const { result } = renderHook(() => useExpedicao(), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data).toHaveLength(1);
    expect(result.current.data?.[0].cliente).toBe('Cliente A');
  });
  ...
  test('propaga erro quando Supabase falha', async () => {
    ...
    expect(result.current.error?.message).toContain('Erro ao carregar expedicao');
  });
});
