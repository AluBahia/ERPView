...
describe('usePagar', () => {
  ...
  test('retorna titulos a pagar com sucesso', async () => {
    const mockData = [{ id: 1, numero: 'TP001', fornecedor: 'Forn A', valor: 2000, status: 'Pendente' }];
    mockSupabaseClient.from.mockImplementation(() => mockBuilder(mockData));

    const { result } = renderHook(() => usePagar(), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data).toHaveLength(1);
    expect(result.current.data?.[0].fornecedor).toBe('Forn A');
  });
  ...
  test('propaga erro quando Supabase falha', async () => {
    ...
    expect(result.current.error?.message).toContain('Erro ao carregar titulos a pagar');
  });
});
