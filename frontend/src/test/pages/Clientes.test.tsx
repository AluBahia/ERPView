...
vi.mock('../../hooks/useClientes', () => ({
  useClientes: () => ({ data: [], isLoading: false, error: null, refetch: vi.fn() }),
}));
...
describe('Clientes', () => {
  test('renderiza sem crash', async () => {
    const { default: Clientes } = await import('../../pages/Clientes');
    const { container } = render(<Clientes />, { wrapper: createWrapper() });
...
describe('Clientes', () => {
  test('renderiza conteudo', async () => {
    const { default: Page } = await import('../../pages/Clientes');
