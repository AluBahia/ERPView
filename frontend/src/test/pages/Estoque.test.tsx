...
vi.mock('../../hooks/useEstoque', () => ({ useEstoque: () => ({ data: [], isLoading: false, error: null, refetch: vi.fn() }) }));
...
describe('Estoque', () => {
  test('renderiza sem crash', async () => {
    const { default: Estoque } = await import('../../pages/Estoque');
    const { container } = render(<Estoque />, { wrapper: createWrapper() });
...
describe('Estoque', () => {
  test('renderiza conteudo', async () => {
    const { default: Page } = await import('../../pages/Estoque');
